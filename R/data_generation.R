...
data_generation = function(n, p, rho,
  ...
  if(response == 'survival'){
    f = 2 * ((3 * x[, 1] - 2)^2 - 1.5) + 3 * f2(x[,2]) + 1 * sin(2 * pi * x[, 3]) + 3 * ((x[, 4] - 0.4)^2 - 0.15) + 4 * f5(x[,5]) # 가장 잘 된 세팅?
    # f = 2 * f2(x[, 1]) + 1 * f4(x[,2]) + 3 * sin(2 * pi * x[, 3]) + 4 * ((x[, 4] - 0.4)^2 - 0.15) + 3 * f5(x[,5]) # 가장 괜찮은 세팅
    # f = 6 * f2(x[, 1]) + 4 * f4(x[,2]) + 3 * sin(2 * pi * x[, 3]) + 4 * ((x[, 4] - 0.4)^2 - 0.15) + 5 * f5(x[,5]) # 가장 괜찮은 세팅에서 변형
