...
cdcosso.glm = function (x, y, f, wt, nbasis, basis.id, lambda0, lambda_theta, gamma, obj, type, kparam, scale)
{
  n = length(y)
  p = length(wt)
  ...
  par(mfrow = c(1,2))
  # solve (theta) - 1st
  sspline_cvfit = cv.sspline.subset(K, y, f, nbasis, basis.id, rep(1, p)/wt^2, lambda0, obj, type, kparam, one.std = TRUE,  show = TRUE) ## 초기값 설정. 수정할 함수
  ...
  # solve (theta) - 2nd
  sspline_cvfit = try({cv.sspline.subset(K, y, f, nbasis, basis.id, rep(1, p)/wt^2, lambda0, obj, type, kparam, one.std = FALSE, show = FALSE)}) ## 초기값 설정. 수정할 함수
