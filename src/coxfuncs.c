...
SEXP cox_c_step(SEXP zw, SEXP Rw, SEXP cw, SEXP sw, SEXP n, SEXP lambda0) {
  int nc = INTEGER(n)[0];
  ...
  for (iter = 0; iter < 10; ++iter) {
    ...
    for (int j = 0; j < nc; ++j) { // iterate by column
      ...
      double V4 = Rw_c[j * nc + j] / (2 * lambda0_c);
...
SEXP cox_theta_step(SEXP Gw, SEXP uw, SEXP n, SEXP d, SEXP theta, SEXP lambda_theta, SEXP gamma) {
  // Convert R vectors to C arrays
